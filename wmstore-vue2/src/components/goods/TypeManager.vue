<template>
  <div>
    <div style="margin-bottom: 5px">
      <el-input placeholder="请输入分类名字" prefix-icon="el-icon-search" v-model="name" style="width: 200px; margin-left: 5px"
        @keyup.enter.native="loadPost()">
      </el-input>

      <el-select v-model="status" placeholder="请选择分类状态" style="margin-left: 5px" v-if="user.role == 0">
    ...
    <el-table ref="multipleTable" :data="tableData" tooltip-effect="dark" border style="width: 100%" stripe
      ...
      <el-table-column prop="remark" label="分类介绍" width="400">
      </el-table-column>
      ...
      <el-table-column fixed="right" label="操作" prop="operate">
        ...
        <template slot-scope="scope">
          ...
          <el-switch :value="scope.row.status == 1" active-color="#13ce66" inactive-color="#ff4949" active-text="正常"
            inactive-text="禁用" @change="modStatus(scope.row.id)">
          </el-switch>
          <el-divider direction="vertical"></el-divider>
  ...
  methods: {
    ...
    mod(row) {
      ...
      this.$nextTick(() => {
        //赋值到表单
        this.$axios
          .get(this.$httpUrl + "/type/info/" + row.id)
          .then((res) => {
    ...
    doSave() {
      this.$axios
        .post(this.$httpUrl + "/type/save", this.form)
        .then((res) => res.data)
        .then((res) => {
          console.log(res);
          if (res.code == 200) {
    ...
    doMod() {
      this.$axios
        .post(this.$httpUrl + "/type/update", this.form)
        .then((res) => res.data)
        .then((res) => {
          console.log(res);
          if (res.code == 200) {
    ...
    loadPost() {
      this.$axios
        .post(this.$httpUrl + "/type/listPage", {
    ...
    modStatus(id) {
      this.$axios
        .post(this.$httpUrl + "/type/control/" + id)
        .then((res) => res.data)
        .then((res) => {
          console.log(res);
          if (res.code == 200) {
    ...
    delHandle(id) {
      ...
      console.log(ids);
      this.$confirm("将删除分类一切有关数据， 是否继续?", "提示", {
        ...
        .then(() => {
          this.$axios
            .post(this.$httpUrl + "/type/deleteBatch", ids, {
              headers: {
